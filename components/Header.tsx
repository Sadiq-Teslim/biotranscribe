...
import { Dna, Activity, History, Home, BookOpen, Cpu } from 'lucide-react';
...
const NavLink: React.FC<{ to: string; icon: React.ReactNode; label: string; active: boolean }> = ({ to, icon, label, active }) => (
  <Link
    to={to}
    className="relative px-3 py-2 rounded-full group transition-colors"
  >
    {active && (
      <motion.div
        layoutId="nav-pill"
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 shadow-lg shadow-slate-200/20 rounded-2xl px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <NavLink to="/about" icon={<BookOpen size={16} />} label="Concepts" active={location.pathname === '/about'} />
            <NavLink to="/architecture" icon={<Cpu size={16} />} label="System" active={location.pathname === '/architecture'} />
          </nav>
          ...
          <div className="lg:hidden flex gap-2">
             ...
             <Link to="/history" className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg">
               <History size={20} />
             </Link>
          </div>
        </div>
